...
On voit qu il ouvre le fichier de password du user level03, le lit et le stock dans un buffer mais le code ne contient pas de buffer overflow possible, par contre la partie suivante attire notre attention :  
...
on peut donc lire dans la stack grace au flag "%p" :  
```bash
...
```
on peut convertir ces adresses en ASCII en prenant le soin de retirer le "0x" jusqu'a ce qu'on tombe sur des valeurs ASCII interessantes et comme l'elf est en little endian, il faut inverser les valeurs  
```text
...
On concatene le tout et on obient notre flag : Hh74RPnuQ9sa5JAEXgNWCqz7sXGnh5J5M9KfPg3H